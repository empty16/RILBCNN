...
local LBORConv, parent = torch.class('nn.LBORConv', 'nn.Module')
...
function LBORConv:reset()
    -- Here first generate the binary filters, and rotate the filters, not generate as a whole 

    local numElements = self.kW * self.kH * self.nInputPlane * self.nOutputPlane * self.nOrientation
    self.weight = torch.CharTensor(self.nOutputPlane, self.nInputPlane, self.nOrientation, self.kW, self.kH):fill(0)
    ...
    self.bias = nil
    self.gradBias = nil
    self.gradWeight = torch.CharTensor(self.nOutputPlane, self.nInputPlane, self.nOrientation, self.kW, self.kH):fill(0)     
end
...
function LBORConv:createInitFuncs()
    self.initFuncs = {
        fast = function ()
            self:clearState()
...
function LBORConv:getMappingIndices()
    local kernelIndices = {
        -- 1x1
        [1] = {
            [0] = {1},
...
function LBORConv:getInterpolationIndices(direction)
    local srcW
    local srcH
    local dir
...
function LBORConv:getDFTFactors(nOrientation, nRotation, direction)
...
function LBORConv:general(extension)
    self.mode = 'general'
