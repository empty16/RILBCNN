...
return function (opt)
   ...
   local provider = torch.load('./datasets/mnist-rot-12k.t7')
   local trainData = {
     data = provider.trainData.data:clone(),
     labels = provider.trainData.labels:clone()
   }
   local testData = {
     data = provider.testData.data:clone(),
     labels = provider.testData.labels:clone()
   }
    
   -- split train data to train + validation set
   local shuffle = torch.randperm(12000)
   provider.trainData = {
      data = torch.DoubleTensor(10000, 1, 28, 28),
      labels = torch.ByteTensor(10000)
    }
   provider.validationData = {
      data = torch.DoubleTensor(2000, 1, 28, 28),
      labels = torch.ByteTensor(2000)
    }
   provider.testData = {
      data = torch.DoubleTensor(50000, 1, 28, 28),
   ...
   -- test dataset
   for i = 1, 50000 do
      provider.testData.data[i]:copy(testData.data[i])
      provider.testData.labels[i] = testData.labels[i]
   end
   testData = nil
