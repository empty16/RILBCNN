...
return function (opt)
   ...
   local provider = torch.load('./datasets/Outex-24-ori.t7')
   local trainData = {
   ...
   local shuffle = torch.randperm(480)
   provider.trainData = {
      data=torch.DoubleTensor(400, 3, 32, 32),
      labels=torch.ByteTensor(400)
    }
   provider.validationData = {
      data=torch.DoubleTensor(80, 3, 32, 32),
      labels=torch.ByteTensor(80)
    }
   for i = 1, 400 do
