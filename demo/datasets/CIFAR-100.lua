...
return function (opt)
   ...
   local provider = torch.load('./datasets/cifar-100.t7')
   local trainData = {
   ...
   -- CIFAR-100 train 50000, 3, 32, 32(42000, 8000), test 10000
   local shuffle = torch.randperm(50000)
   provider.trainData = {
      data = torch.DoubleTensor(42000, 3, 32, 32),
      labels = torch.ByteTensor(42000)
    }
   provider.validationData = {
      data = torch.DoubleTensor(8000, 3, 32, 32),
      labels = torch.ByteTensor(8000)
    }
   ...
   -- auto detection to get the meta info
