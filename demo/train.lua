...
-- use the dofile function to load the dataset
...
if not opt.onlyTest and opt.trainMode == 'gpu' and 
    ...
    for file in paths.files(opt.savePath) do
        if string.find(file, '.t7') then
            bestSavePath = paths.concat(opt.savePath, file)
            model = torch.load(bestSavePath)
        end
    end
end
